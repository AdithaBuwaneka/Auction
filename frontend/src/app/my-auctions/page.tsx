...
import { Plus, Package, DollarSign, TrendingUp, Clock, Edit, Trash2, Upload, X } from 'lucide-react';
...
export default function MyAuctionsPage() {
  ...
  const handleEdit = async (auctionId: number) => {
    try {
      ...
      // Format datetime for input fields (remove seconds and timezone)
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      // Build auction data with all fields
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.response?.data?.error || 'Failed to delete auction. Please try again.';
      alert(errorMessage);
    } finally {
      setDeleting(false);
    }
  ...
  const canEditOrDelete = (auction: any) => {
    // Show edit/delete icons on ALL user's auctions regardless of status
    return true;
  };
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {auctions
                      ...
                      .map((auction) => renderAuctionWithActions(auction))}
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-2 gap-2 mb-3 p-2.5 bg-gray-50 rounded-md text-xs border border-gray-200">
              ...
              {editModal.auction.seller && (
                <div>
                  ...
                  <p className="font-semibold text-gray-900">{editModal.auction.seller.username || 'You'}</p>
                </div>
              )}
            </div>
